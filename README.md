# Accelerating Local Outlier Detection from Data Streams with Local Skipping

This is the implementation of the paper published in KDD 2020 (to appear).

## 1. Overview
Real-time outlier detection from a data stream is becoming an increasingly important problem, especially as sensor generated data streams abound in many applications owing to the prevalence of IoT and the emergence of digital twins. Several density-based approaches have been proposed to address this problem, but, arguably, none of them is fast enough to meet the performance demand of real-world applications. This paper is founded upon a novel observation that, in many regions of the data space, the data densities hardly change across window slides. We propose a new algorithm, abbreviated as STARE, which identifies local regions in which data densities hardly change and skip updating density distribution in those regions. Two techniques, grid-based kernel centers and significance-based localization, are employed to implement the local region skipping idea efficiently and effectively. Extensive experiments using synthetic and real data streams and a case study show that STARE is several orders of magnitude faster than the existing algorithms while achieving comparable or higher accuracy.
